...
class AileronTestCase(unittest.TestCase):
    ...
    def test_invalid_step_size(self):
        with patch('FlaskWebServer.droneObj') as mock_droneObj:
            response = self.client.post('/rudder', json={'body': '0.2', 'stepSize': '8'})
    ...
    def test_invalid_rudder(self):
        with patch('FlaskWebServer.droneObj') as mock_droneObj:
            response = self.client.post('/rudder', json={'body': '1.2', 'stepSize': '1'})
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json, {'message': 'Invalid input for rudder, only float values between -1 and 1 both inclusive are accepted'})
            mock_droneObj.assert_not_called()

    def test_drone_not_armed(self):
        response = self.client.post('/rudder', json={'body': '0.5', 'stepSize': '1'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, {'message': 'Please arm the drone first'})
    
    def test_invalid_input(self):
        with patch('FlaskWebServer.droneObj') as mock_droneObj:
            response = self.client.post('/rudder', json={'body': 'InvalidInput', 'stepSize': '1'})
            self.assertEqual(response.status_code, 200)
    ...
    def test_invalid_keyInput(self):
        with patch('FlaskWebServer.droneObj') as mock_droneObj:
            response = self.client.post('/rudder', json={'bodies': 'IncThrottle', 'stepSize': 'Invalid'})
            self.assertEqual(response.status_code, 200)
